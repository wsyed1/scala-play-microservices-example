...
import models.{FavouriteStudio, Friend, FriendTest}
...
object FriendDAOTest {
  ...
  def index(userId: Int) : List[FriendTest] = {
    ...
    var friends = new ListBuffer[FriendTest]()

    while (rs.next()) {
      friends.+=(new FriendTest(rs.getString("friends").toInt))
    }
